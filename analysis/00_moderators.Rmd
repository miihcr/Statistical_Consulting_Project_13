...
png(p_fig("sem-plots", "semplot_onefactor.png"), width = 7, height = 5, res = 300,
    units = "in")
...
# Visualize the measurement model
png(p_fig("sem-plots", "semplot_twofactor.png"), width = 7, height = 5, res = 300,
    units = "in")
...
png(p_fig("sem-plots", "semplot_minus2.png"), width = 7, height = 5, res = 300,
    units = "in")
...
for (cls in classes) {
  ...
  avg_outdeg <- ifelse(n_nodes > 0, n_edges / n_nodes, NA)
  
  capped_den <- ifelse(n_nodes > 0, n_edges / (max_outdegree * n_nodes), NA)
...
data$cohesion_z <- as.numeric(scale(data$cohesion_capped, center = TRUE, scale = TRUE))
...
cor.test(network_summary$n_nodes, network_summary$cohesion_capped) # support that it is size-invariant 

# Save
saveRDS(data, p_processed("data2_incl_moderation.rds"))

saveRDS(network_summary, p_processed("network_summary.rds"))
