...
m_cohesion_test_no_class <- glmmTMB(
  choice ~ target * group * block + cohesion_capped_c + 
           reward * effort + school +
    (1 + block | ppn),
  data = df_long, 
  family = binomial
...
m_cohesion_3c <- glmmTMB(
    choice ~ (group * block * target) + 
             (group * block * cohesion_capped_c ) +
             (group * target * cohesion_capped_c ) + 
             (block * target * cohesion_capped_c ) +
             reward * effort + school +
      (1 + block | ppn),
    data = df_long, 
    family = binomial
...
m_cohesion_3d <- glmmTMB(
    choice ~ group * block * target * cohesion_capped_c  + 
             reward * effort + school +
      (1 + block | ppn),
    data = df_long, 
    family = binomial
...
m_cohesion_final <- m_cohesion_3a
...
p_cohesion_final <- sjPlot::plot_model(
  m_cohesion_final,
  type = "est",   # <- this is what removes stuff
...
Decision: The best fitting model is m_cohesion_3a
