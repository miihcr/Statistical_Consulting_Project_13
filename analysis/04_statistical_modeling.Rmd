...
Decision: The best fitting model is m_cohesion_3c

#### Reciprocal Density

```{r}
# use reciprocal
df_long$cohesion_z_recip <- scale(df_long$cohesion_recip)[, 1]

# Model 4a: Base model with cohesion main effect

m_cohesion_4a <- glmmTMB(
  choice ~ target * group * block + cohesion_z_recip 
  + reward * effort + school +
      (1 + block | ppn),
    data = df_long, 
    family = binomial
...
# Model 4b: Add Group × Block × Cohesion

m_cohesion_4b <- glmmTMB(
    choice ~ target * group * block + 
             group * block * cohesion_z_recip + 
             reward * effort + school +
      (1 + block | ppn),
    data = df_long, 
    family = binomial
...
# Model 4c: Add three-way interactions with target

m_cohesion_4c <- glmmTMB(
    choice ~ (group * block * target) + 
             (group * block * cohesion_z_recip) +
             (group * target * cohesion_z_recip) + 
             (block * target * cohesion_z_recip) +
             reward * effort + school +
      (1 + block | ppn),
    data = df_long, 
    family = binomial
...
# Model 4d: Full four-way interaction

m_cohesion_4d <- glmmTMB(
    choice ~ group * block * target * cohesion_z_recip + 
             reward * effort + school +
      (1 + block | ppn),
    data = df_long, 
    family = binomial
...
print(anova(m_cohesion_4a, m_cohesion_4b, m_cohesion_4c, m_cohesion_4d))

summary(m_cohesion_4b)
```
